...
public class GameServer {
    ...
    static class GameState {
        ...
        final List<int[]> personalCards = new ArrayList<>(); // 각 플레이어의 3장 카드
        final List<Integer> sharedCards = new ArrayList<>();  // 공유 카드 4장
    ...
    static class ClientHandler implements Runnable {
        ...
        void handleMessage(String msg) {
            ...
            switch (cmd) {
                ...
                case "CREATE_ROOM":
                    // CREATE_ROOM|roomName|maxPlayers|password(옵션)
                    handleCreateRoom(parts);
                    break;

                case "JOIN_ROOM":
                    // JOIN_ROOM|roomCode|password(옵션)
                    handleJoinRoom(parts);
                    break;

                case "QUICK_JOIN":
                    // QUICK_JOIN|maxPlayers
                ...
                case "BET_ACTION":
                    // BET_ACTION|CALL 또는 BET_ACTION|FOLD
                    if (currentRoom != null && currentRoom.gameState != null) {
        ...
        void handleCreateRoom(String[] parts) {
            // CREATE_ROOM|roomName|maxPlayers|password
            String name = parts[1];
